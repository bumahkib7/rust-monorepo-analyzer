...
const SQL_PATTERNS: &[&str] = &[
    "query", "execute", "exec_sql", "raw_sql", "cursor", "rawquery",
];
const RAW_HTML: &[&str] = &[
    "innerhtml",
    "outerhtml",
...
impl SafeReason {
    pub fn description(&self) -> &'static str {
        match self {
            ...
            SafeReason::ArgumentArrayConstantBinary => {
                "Command with constant binary and args array"
            }
...
fn matches_shell_invocation(name: &str, language: Language) -> bool {
    ...
    let lang_specific = match language {
        ...
        Language::Php => {
            name.contains("shell_exec") || name.contains("passthru") || name.contains("proc_open")
        }
...
fn matches_binary_taint_pattern(name: &str, language: Language) -> bool {
    ...
    match language {
        ...
        Language::Python => name.contains("subprocess.run") && !name.contains("[\""),
        Language::JavaScript | Language::TypeScript => {
...
fn matches_args_based_exec(name: &str, language: Language) -> bool {
    match language {
        ...
        Language::Python => name.contains("subprocess.run") && !name.contains("shell=true"),
        Language::Go => name.contains("exec.command"),
...
fn matches_sql_sink(name: &str, language: Language) -> bool {
    ...
    let lang_specific = match language {
        Language::Java | Language::Kotlin => {
            name.contains("createstatement") || name.contains("executequery")
        }
...
fn matches_raw_html_sink(name: &str, language: Language) -> bool {
    let common =
        matches_patterns(name, RAW_HTML) || name == "html" || name.contains("insertadjacenthtml");
    let lang_specific = match language {
...
fn matches_url_sink(name: &str, language: Language) -> bool {
    ...
    let lang_specific = match language {
        Language::JavaScript | Language::TypeScript => {
            name.contains("window.location") || name.contains("window.open")
        }
        Language::Python => name.contains("redirect(") || name.contains("httpresponseredirect"),
        Language::Java | Language::Kotlin => {
            name.contains("sendredirect") || name.contains("forward")
        }
...
fn matches_template_sink(name: &str, language: Language) -> bool {
    match language {
        ...
        Language::JavaScript | Language::TypeScript => {
            name.contains("ejs.render") || name.contains("handlebars")
        }
        Language::Java | Language::Kotlin => {
            name.contains("freemarker") || name.contains("velocity")
        }
    ...
    match context {
        SinkContext::CommandShell => {
            let lang_specific = match language {
                ...
                Language::JavaScript | Language::TypeScript => {
                    "Use spawn with args array, not exec string"
                }
        ...
        SinkContext::CommandExecArgs => {
            "Ensure binary is constant/allowlisted. Validate args for flags like -c, --;"
                .to_string()
        }
        SinkContext::CommandBinaryTaint => {
            "Never execute user-controlled binary paths. Use allowlist or fixed command map."
                .to_string()
        }
        ...
        SinkContext::Sql => format!("Use {} instead of string concatenation", sanitizer_list),
        SinkContext::Url => format!(
            "Validate URL scheme (reject javascript:). Recommended: {}",
            sanitizer_list
        ),
        SinkContext::Template => format!("Enable auto-escaping. Recommended: {}", sanitizer_list),
        SinkContext::FilePath => {
            let lang_specific = match language {
                ...
                Language::JavaScript | Language::TypeScript => {
                    "Use path.resolve(), check path.startsWith(baseDir)"
                }
